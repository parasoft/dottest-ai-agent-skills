...
- If any new violations were introduced by the fix - attempt to fix them, and verify. Number of diffrent fix approaches to attempt is defined by `DOTTEST_FIX_ATTEMPTS` environment variable. If not set, default is 2 attempts (1 original fix + 1 retry with a different approach). If all attempts fail, revert all changes and move on to the next violation.
